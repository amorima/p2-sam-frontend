...
const { data: patronsData, status: patronsStatus } = await useFetch<{ data: Patron[] }>('/api/patrons', {
  lazy: true,
  server: false
})
...
const selectedPatron = computed(() =>
  patrons.value.find(p => p.resource.nif_nipc === selectedNif.value)
)

const docNumber = useState('docNumber', () => {
...
const schema = z.object({
  data: z.string().min(1, 'Data de emissão obrigatória'),
...
watch(selectedPatron, (patron) => {
  if (!patron) return
  state.nome_entidade = patron.entity.nome_entidade
  state.nif_nipc = patron.entity.nif_nipc
  state.email = patron.entity.email_login
  state.iban = patron.entity.iban
  const loc = patron.locations[0]
  if (loc) {
    state.rua = loc.rua
...
const estadoBadgeColor = computed(() => {
  const map: Record<EstadoDonativo, 'warning' | 'success' | 'error'> = {
    PENDENTE: 'warning',
    ACEITE: 'success',
    REJEITADO: 'error'
  }
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  if (!selectedNif.value) {
    toast.add({
      title: 'Mecenas em falta',
      description: 'Selecione um mecenas antes de registar a doação.',
  ...
  try {
    ...
    toast.add({
      ...
      description: 'A doação foi registada com sucesso.',
      icon: 'i-lucide-check',
      color: 'success'
    })
    ...
    toast.add({
      ...
      description: 'Não foi possível registar a doação. Tente novamente.',
      icon: 'i-lucide-x',
      color: 'error'
    })
  } finally {
...
function openMBModal() {
  if ((state.valor_transacao ?? 0) <= 0) {
    toast.add({
      title: 'Valor em falta',
      description: 'Introduza um valor antes de gerar a referência.',
      icon: 'i-lucide-alert-circle',
      color: 'warning'
    })
    return
...
<template>
  <UDashboardPanel id="donations-new">
    <template #header>
      <UDashboardNavbar title="Nova Doação">
        ...
        <template #right>
          <UBadge
            ...
            variant="subtle"
            size="lg"
            class="hidden sm:flex capitalize"
          >
            {{ state.estado }}
          </UBadge>
          <UButton
            ...
            <UPageCard variant="subtle" class="h-full">
              <template #header>
                <div class="flex items-center justify-between">
                  ...
                  <UButton
                    v-if="selectedNif"
                    :label="isEditingClient ? 'Fechar edição' : 'Editar dados'"
                    :icon="isEditingClient ? 'i-lucide-x' : 'i-lucide-pencil'"
                    color="neutral"
                    variant="ghost"
                    size="sm"
                    @click="isEditingClient = !isEditingClient"
                  />
                </div>
              </template>

              <div class="space-y-4">
                <UFormField label="Selecionar Mecenas">
                  <USelect
                    v-model="selectedNif"
                    :items="patronOptions"
                    value-key="value"
                    ...
                    searchable
                    search-placeholder="Pesquisar mecenas..."
                    class="w-full"
                  />
                </UFormField>

                <template v-if="selectedNif">
                  <USeparator />

                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <UFormField name="nome_entidade" label="Nome / Razão Social" class="sm:col-span-2">
                      <UInput
                        v-model="state.nome_entidade"
                ...
                <template v-else>
                  <div class="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p class="text-sm text-muted">
                      Selecione um mecenas para preencher os dados do cliente.
                    </p>
                  </div>
                </template>
              </div>
        ...
        <UPageCard variant="subtle">
          ...
          <div class="space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start rounded-lg border border-default bg-elevated/30 p-4">
              ...
              <UFormField name="metodo_pagamento" label="Método de Doação" required>
                <USelect
              ...
              <UFormField name="anonimo" label="Doação Anónima">
                <div class="flex items-center gap-3 h-[34px]">
                  ...
                  <span class="text-sm text-muted">
                    {{ state.anonimo ? 'O nome do mecenas não será divulgado' : 'Doação pública' }}
                  </span>
                </div>
              </UFormField>
            </div>
        ...
        <UPageCard variant="subtle">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="space-y-1">
              ...
              <p class="text-sm text-muted">
                Donativo monetário
                <template v-if="state.metodo_pagamento">
