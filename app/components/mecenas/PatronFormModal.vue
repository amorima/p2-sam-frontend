...
const isEditing = computed(() => !!props.patron)
...
watch(() => props.open, (isOpen) => {
  if (!isOpen) return
  if (props.patron) {
    const e = props.patron.entity
    const l = props.patron.locations[0]
    const c = props.patron.contacts[0]
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    if (isEditing.value) {
      await $fetch(`/api/patrons/${props.patron!.resource.nif_nipc}`, { method: 'PATCH', body })
    } else {
      await $fetch('/api/patrons', { method: 'POST', body })
    }

    toast.add({
      title: isEditing.value ? 'Mecenas atualizado' : 'Mecenas criado',
      description: isEditing.value
        ? 'Os dados do mecenas foram atualizados.'
        : `Mecenas ${event.data.nome_entidade} criado com sucesso.`,
      icon: 'i-lucide-check',
      color: 'success'
    })

    emit('saved', event.data.nif_nipc)
    emit('update:open', false)
  } catch {
    toast.add({
      title: 'Erro',
      description: isEditing.value ? 'Não foi possível atualizar o mecenas.' : 'Não foi possível criar o mecenas.',
      icon: 'i-lucide-x',
      color: 'error'
    })
  } finally {
...
<template>
  <UModal
    :open="open"
    :title="isEditing ? 'Editar Mecenas' : 'Novo Mecenas'"
    :description="isEditing ? `A editar: ${patron?.entity.nome_entidade}` : 'Preencha os dados para registar um novo mecenas.'"
      ...
      >
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <UFormField name="nome_entidade" label="Nome / Razão Social" required>
            ...
            <UFormField name="password" :label="isEditing ? 'Nova Password (opcional)' : 'Password'" :required="!isEditing">
              <UInput
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <UFormField name="descricao_contacto" label="Função">
            <UInput v-model="state.descricao_contacto" class="w-full" placeholder="Responsável" />
          </UFormField>
        </div>
        ...
        <div class="flex justify-end gap-2 pt-2">
          ...
          />
          <UButton
            :label="isEditing ? 'Guardar alterações' : 'Criar Mecenas'"
